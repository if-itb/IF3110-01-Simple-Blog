<?php
	...
	$query = "UPDATE `post` SET `Title`=\"".$_POST["Judul"]."\",`Content`=\"".$_POST["Konten"]."\" WHERE PostID=".$_GET['id'];
	$result = mysqli_query($con,$query) or die("Unable to execute query");
	$query = "UPDATE `simpleblog`.`post` SET `Date` = '".to_date_type($_POST['Tanggal'])."' WHERE `post`.`PostID` = ".$_GET['id'].";" ;
	$result = mysqli_query($con,$query) or die("Unable to execute query");
	mysqli_close($con); 
