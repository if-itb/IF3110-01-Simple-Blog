    ...
    <div class="posts">
        <nav class="art-list">
		  ...
		  include 'ConvertDateFunc.php';
				$postQuery= 'SELECT * FROM `tucilDB_13511097`.`listpost` ORDER BY `date` DESC';
				$postResult =  mysql_query($postQuery,$con);
				while($search_row = mysql_fetch_array($postResult)) {
					$id_post = $search_row['id'];
					$title = $search_row['title'];
					$sqldate = $search_row['date'];
					$date = ConvertDate($sqldate);
					$isi = $search_row['post'];
					
					echo'
						<li class="art-list-item">
							<div class="art-list-item-title-and-time">
								<h2 class="art-list-title"><a href="post.php?id='.$id_post.'">'.$title.'</a></h2>
								<div class="art-list-time">'.$date.'</div>
							</div>
							<p>'.$isi.'</p>
							<p><a href="new_post.php?mode=1&id_post='.$id_post.'">Edit</a> | <a href="javascript:ConfirmDelete('.$id_post.')">Hapus</a>
						</p>
						</li>
					';
				}
