	...
	<div class="posts">
        <nav class="art-list">
          <ul class="art-list-body">
			<?php
				try {
					$stmt = $db->query('SELECT id, title, content, date FROM post ORDER BY id DESC');
					while($row = $stmt->fetch()){
						echo '<li class="art-list-item">';
							echo '<div class="art-list-item-title-and-time">';
								echo '<h2 class="art-list-title">
									<a href="view_post.php?id='.$row['id'].'">'.$row['title'].'</a></h2>';
								echo '<div class="art-list-time">'.date('jS M Y H:i:s', strtotime($row['date'])).'</div>';
							echo '</div>';	
							echo '<p>'.$row['content'].'</p>';				
							echo '<p><a href="edit_post.php?id='.$row['id'].'">Edit</a>';
			?>
							<a href="javascript:deletePost('<?php echo $row['id'];?>')">|  Hapus</a></p>															
			<?php
						echo '</li>';
						}
